...
class TransientVolpathSimpleIntegrator(mi.ad.integrators.common.RBIntegrator):
    """Simplified volumetric path tracer with support for Differential Delta Tracking.
    Modified to support transient image data.
    ...
    def render(self: mi.SamplingIntegrator,
        ...
        with dr.suspend_grad():
            ...
            # Prepare the films for the transient sequence
            self.prepare_transient(sensor=sensor, aovs=self.aovs())
            ...
            # Produce the primal image
            block = sensor.film().create_block() # Prepare an ImageBlock as specified by the film
            block.set_coalesce(block.coalesce() and spp >= 4) # Only use the coalescing feature when rendering enough samples

            alpha = dr.select(valid, mi.Float(1), mi.Float(0))
            block.put(pos, ray.wavelengths, L * weight, alpha) # Accumulate into the image block
            
            sensor.film().put_block(block) # Perform the weight division and return an image tensor
            self.primal_image = sensor.film().develop()
            
            n_images = sensor.num_films()
            height, width, n_channels = dr.shape(self.primal_image)
            images = dr.zeros(mi.TensorXf, (n_images + 1, height, width, n_channels))
            images[0] = self.primal_image.array

            # Produce the transient image sequence
            dt = self.dt
            for i in range(n_images):
                # Get info for this time range
                t1 = i * dt; t2 = t1 + dt
                within_range = np.all([self.path_lengths > t1, self.path_lengths < t2], axis=0)
                Li = dr.select(mi.Mask(within_range), L, 0)

                block_i = sensor.film().create_block()
                block_i.set_coalesce(block_i.coalesce() and spp >= 4)
                alpha = dr.select(valid, mi.Float(1), mi.Float(0))
                block_i.put(pos, ray.wavelengths, Li * weight, alpha)
                sensor.film().put_block_transient(block_i, i)
                images[i+1] = sensor.film().develop_transient(i).array
    ...
    def render_backward(self: mi.SamplingIntegrator,
        ...
        *gradient image* (``grad_in``) and transforms and accumulates these into
        ...
        with dr.suspend_grad():
            ...
            block = film.create_block() # Prepare an ImageBlock as specified by the film
            block.set_coalesce(block.coalesce() and spp >= 4) # Only use the coalescing feature when rendering enough samples
            ...
            with dr.resume_grad():
                ...
                for i in range(n_images):
                    ...
                    blocks.append(sensor.film().create_block())
                    blocks[i].set_coalesce(blocks[i].coalesce() and spp >= 4)
                    blocks[i].put(
                        pos=pos,
                        wavelengths=ray.wavelengths,
                        value=Li * weight * det,
                    ...
                    sensor.film().put_block_transient(blocks[i], i)
                ...
                n_images = sensor.num_films()
                height, width, n_channels = dr.shape(image)
                images = dr.zeros(mi.TensorXf, (n_images + 1, height, width, n_channels))
                images[0] = image.array
                for i in range(n_images):
                    dr.schedule(state_out, blocks[i].tensor())
                    images[i+1] = sensor.film().develop_transient(i).array

                # Differentiate sample splatting and weight division steps to
                # retrieve the adjoint radiance
                dr.set_grad(images, grad_in)
                dr.enqueue(dr.ADMode.Backward, images)
    ...
    def sample(self,
        ...
        if path_state is not None:
            ...
            path_length = path_state.path_length()
            si = path_state.si()
            escaped = path_state.escaped()
        else:
            active = mi.Mask(True)
            depth = dr.zeros(mi.Int32, wavefront_size)
            path_length = dr.zeros(mi.Float, wavefront_size)
        ...
        # test = np.array(active)
        # print(test.shape)
        # print(16384 - np.sum(test))
        ...
        loop = mi.Loop("TransientVolpathSimpleSampleLoop", lambda: (
            active, escaped, depth, path_length, ray, throughput, si, result, sampler, alt_sampler,
        ...
        while loop(active):
            ...
            if not primal:
                if self.use_drt:
                    ...
                    self.backpropagate_scattering_drt(
                        scene, medium, alt_sampler, ray, si, throughput, depth, path_length, 
            ...
            # --- Update path length
            # t = np.array(Wrapper(si.t))
            # not_inf = np.invert(np.isinf(t))
            # print(mei.t)
            path_length[did_scatter] = path_length + mei.t # TODO
            # ----------
        ...
        self.path_lengths = np.array(path_length)
        # test = np.array(path_length)
        # print(test.shape)
        # print(test[test!=0].shape)
        # t = np.array(si.t)
        # print("t", np.unique(t))
        # not_inf = np.invert(np.isinf(t))
        # print(t[not_inf].shape)
        # print(t[t==inf].shape)
        ...
        if (not primal) and self.use_drt and self.use_drt_subsampling:
            ...
            self.backpropagate_scattering_drt(
                ...
                None, drt_state.depth(), drt_state.path_length(), channel,
    ...
    def prepare_transient(self,
                         sensor: mi.Sensor,
                         aovs: list = []):
        for i in range(sensor.num_films()):
            sensor.film().prepare(aovs)
    ...
    def estimate_transmittance(self, ray_full, tmin, tmax, medium, sampler, channel, active,
        ...
        loop = mi.Loop("TransientVolpathSimpleNEELoop", lambda: (active, ray, tmax, transmittance, sampler))
        while loop(active):
    ...
    # modified to include path_length # TODO
    def backpropagate_scattering_drt(self, scene, medium, alt_sampler, ray, si,
                                     throughput, depth, path_length, channel,
        ...
        with dr.suspend_grad():
            drt_Li = self.sample_recursive(
                ...
                mei_sub, channel, depth, path_length, active)
        # ----------
    ...
    def sample_recursive(self, scene, alt_sampler, medium, ray, si, mei, channel,
        ...
        next_path_length = dr.select(active, path_length + 1., path_length) # TODO
...
# added path_length field
class PathState():
    """
    Helper structure holding path state information needed to
    trace recursive rays.
    """
    def __init__(self, depth=None, path_length=None, si=None, last_scatter_direction_pdf=None,
                 escaped=None, active=None, n=None):
        # TODO: would need medium pointer as well if we didn't assume a single medium
        if n is not None:
            assert (depth, path_length, si, last_scatter_direction_pdf, escaped) == (None, None, None, None, None)
            assert active is not None
            self._depth = dr.full(mi.Int32, -1, n)
            self._path_length = dr.full(mi.Float, 0, n)
    ...
    def loop_put(self, loop):
        loop.put(lambda: (self._depth, self._path_length, self._si, self._last_scatter_direction_pdf,
                          self._escaped, self._active))

    def set(self, state: dict, enabled):
        self._depth[enabled] = state['depth']
        self._path_length[enabled] = state['path_length']
    ...
    def path_length(self):
        return mi.Float(self._path_length)
    def si(self):
...
mi.register_integrator("transientvolpathsimple", lambda props: TransientVolpathSimpleIntegrator(props))
